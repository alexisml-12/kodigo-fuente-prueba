...
Este documento explica las decisiones de arquitectura y herramientas tomadas para el módulo de gestión de promociones, dentro de los espacios de libertad que dejaba el enunciado.
...
De las tres opciones permitidas (PostgreSQL, SQL Server, MongoDB) se eligió SQL Server porque es la que el candidato ya tiene instalada y con la que tiene más experiencia reciente, lo que reduce el riesgo de errores de configuración dentro del tiempo disponible para la prueba. El modelo de datos es completamente relacional (promociones con estado, tipo de descuento y vigencia), por lo que una base relacional encaja mejor que una orientada a documentos.
...
## Alcance no incluido (a propósito)

Siguiendo la nota de "prioriza calidad sobre cantidad", se dejó fuera intencionalmente:

- Edición completa de una promoción ya creada (el enunciado solo pide crear, listar, cambiar estado y eliminar).
- Autenticación/autorización de usuarios (no estaba en el alcance).
- Un CRUD completo de productos (el catálogo se siembra con datos de ejemplo vía `prisma db seed`, ya que el foco de la prueba es la gestión de promociones).
